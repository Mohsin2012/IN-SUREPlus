<?php 
  ...
  if(isset($_POST['search']))
  {
    ...
    header('location:norm_cursor.php');
  }
?>
  ...
  <div class="col-md-9 mx-auto table-responsive-sm">
    ...
    <table class="table table-sm table-bordered shadow p-3 mb-5 bg-white rounded">
        ...
        <tr>
          ...
          <form method="post" action="">
            <th scope="col" class="text-center">Policy Name
              <div class="btn-group" role="group">
                <button type="submit" name="pndesc" id="desc" class="btn btn-sm btn-outline-dark"><i
                    class="fal fa-sort-down"></i></button>
                <button type="submit" name="pnasc" id="asc" class="btn btn-sm btn-outline-dark"><i
                    class="fal fa-sort-up"></i></button>
              </div>
            </th>
          </form>
          ...
          <form method="post" action="">
            <th scope="col" class="text-center">Policy Company
              <div class="btn-group" role="group">
                <button type="submit" name="pcdesc" id="desc" class="btn btn-sm btn-outline-dark"><i
                    class="fal fa-sort-down"></i></button>
                <button type="submit" name="pcasc" id="asc" class="btn btn-sm btn-outline-dark"><i
                    class="fal fa-sort-up"></i></button>
              </div>
            </th>
          </form>
      ...
      <tbody>
        ...
        $sq = "SELECT * FROM `norm_category` "; 
        if(isset($_POST["pndesc"]))
        {
          $sq = "SELECT * FROM `norm_category` ORDER BY `norm_category`.`Policy_name` DESC"; 
        }
        if(isset($_POST["pnasc"]))
        {
          $sq = "SELECT * FROM `norm_category` ORDER BY `norm_category`.`Policy_name` ASC"; 
        }
        if(isset($_POST["pcdesc"]))
        {
          $sq = "SELECT * FROM `norm_category` ORDER BY `norm_category`.`Policy_company` DESC"; 
        }
        if(isset($_POST["pcasc"]))
        {
          $sq = "SELECT * FROM `norm_category` ORDER BY `norm_category`.`Policy_company` ASC"; 
        }
        ...
        <tr>
          ...
          <td class="text-center"><a href="update_nor_cat.php?id=<?php echo $result['Policy_no']; ?>"> <i
                class="far fa-edit"></i></a>
          </td>
          <td class="text-center"><a href="delete_nor_cat.php?id=<?php echo $result['Policy_no']; ?>"> <i
