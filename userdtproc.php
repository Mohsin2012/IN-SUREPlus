  ...
  <div class="col-md-9 mx-auto">
    ...
    <div class="text-center row shadow p-3 mb-5 bg-white rounded row align-items-center">
      ...
      <div class="input-group my-3 col-md-4 row justify-content-end">
        <a href="userdt.php"> <i class="fa fa-chevron-circle-left" aria-hidden="true"></i></a>
      </div>
    </div>
    
  </div>

  <div class="col-md-9 mx-auto table-responsive-md">
    ...
    <table class="table table-sm table-bordered shadow p-3 mb-5 bg-white rounded">
      <thead class="table-dark">
        <tr>
          ...
          <th scope="col" class="text-center align-middle">Firstname</th>
          <th scope="col" class="text-center align-middle">Lastname</th>
          ...
          <th scope="col" class="text-center align-middle">Reg. date</th>
...
$sq = "SELECT * FROM login INNER JOIN user ON login.Userid=user.Userid WHERE login.Username LIKE '%$user%'";
$query = mysqli_query($con, $sq) or die(mysqli_error($con));

while ($result = mysqli_fetch_assoc($query)) {$a = 1;
    ?>
...
if ($a == 0) {
    ...
    ?>
        <script>
          window.location.href = "userdt.php";
        </script>
        <?php
}
?>
